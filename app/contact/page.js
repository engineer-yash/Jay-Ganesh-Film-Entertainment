...
export default function ContactPage() {
  const [form, setForm] = useState({ name: '', email: '', phone: '', branch: 'hinjewadi', message: '' });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setForm({ name: '', email: '', phone: '', branch: 'hinjewadi', message: '' });
    } catch { toast.error('Something went wrong. Please try again or WhatsApp us.'); }
    setLoading(false);
  };

  return (
    <div className="pt-28 pb-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><Breadcrumbs items={[{ label: 'Contact' }]} /></div>

      <section className="mt-8 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-12 gap-10">
        <div className="lg:col-span-6">
          <Reveal><div className="text-xs uppercase tracking-[0.3em] text-primary">Contact</div></Reveal>
          <Reveal delay={0.1}><h1 className="font-display text-5xl md:text-7xl mt-4 leading-[1.02]">Let’s <span className="text-gradient-gold">talk</span>.</h1></Reveal>
          <Reveal delay={0.2}><p className="mt-6 text-lg text-muted-foreground">Book a trial, register for a workshop, enquire about hall rental, event registration or wedding choreography — drop us a note and we’ll reach out within 24 hours.</p></Reveal>
          <Reveal delay={0.3}>
          ...
          <Reveal delay={0.4}>
            <div className="mt-10 grid sm:grid-cols-2 gap-3">
              {branches.map((b) => (
                <a key={b.slug} href={b.maps} target="_blank" rel="noreferrer" className="p-4 rounded-2xl border border-border bg-card/40 hover:border-primary transition group">
                  <div className="text-[10px] uppercase tracking-widest text-primary">{b.tag}</div>
                  <div className="font-display text-lg mt-1 group-hover:text-primary transition">{b.name}</div>
                  <div className="mt-1 text-xs text-muted-foreground flex items-start gap-1"><MapPin className="h-3 w-3 mt-0.5 shrink-0" /> {b.address}</div>
                </a>
              ))}
            </div>
          </Reveal>
      ...
      <section className="mt-16 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <Reveal>
          <div className="rounded-3xl overflow-hidden border border-border">
            <iframe src="https://www.google.com/maps?q=Akshay+Plaza+Hinjewadi+Phase+1+Pune&output=embed" loading="lazy" className="w-full aspect-[16/9]" title="Jay Ganesh Films — Hinjewadi flagship map" />
          </div>
        </Reveal>
      </section>
    </div>
