  ...
  return (
    <section ref={ref} className="relative min-h-screen flex items-center pt-24 overflow-hidden">
      <motion.div style={{ y, scale, opacity }} className="absolute inset-0 -z-10">
        <Image src="/images/hinjewadi-poster-2.jpg" alt="" fill priority className="object-cover object-center opacity-30" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-12 gap-12 items-center py-20 relative">
        <div className="lg:col-span-7">
          ...
          <h1 className="font-display mt-6 text-5xl sm:text-6xl md:text-7xl lg:text-8xl leading-[0.95] tracking-tight">
            {['Where', 'Dance,', 'Films &', 'Culture', 'come alive.'].map((word, i) => (
              <motion.span key={i} initial={{ opacity: 0, y: 60, rotate: -3 }} animate={{ opacity: 1, y: 0, rotate: 0 }} transition={{ duration: 0.8, delay: 0.15 + i * 0.1, ease: [0.2, 0.7, 0.2, 1] }} className={`inline-block mr-4 ${(word === 'Dance,' || word === 'Culture') ? 'text-gradient-gold' : ''}`}>
                {word}
              </motion.span>
            ))}
          </h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.7 }} className="mt-6 max-w-xl text-lg text-muted-foreground leading-relaxed">
            Jay Ganesh Films Entertainment is a family of premium dance studios, cinematic events and cultural productions — helmed by national dance icon <span className="text-foreground">Mukesh (Rudra) Chaudhari</span>.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.85 }} className="mt-10 flex flex-wrap gap-4">
            <Link href="/branches" className="group inline-flex items-center gap-2 px-6 py-3.5 rounded-full bg-primary text-primary-foreground font-semibold hover:bg-primary/90 transition">Explore Studios <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition" /></Link>
            <Link href="/founder" className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full border border-border hover:border-primary hover:text-primary transition">Meet the Founder</Link>
          </motion.div>
          <div className="mt-14 grid grid-cols-4 gap-6 max-w-lg">
            {founder.stats.map((s, i) => (
              <motion.div key={s.label} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1 + i * 0.1 }}>
